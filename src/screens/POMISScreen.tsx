...
export default function POMISScreen() {
  ...
  const calculate = () => {
    ...
    const res = calcPOMIS(P, R, parseInput(taxRate));
    setResult(res);
  };
          ...
          <View style={styles.section}>
            <ResultCard
              ...
              rows={[
                ...
                { label: 'Post-Tax Monthly Payout', value: result.postTaxMonthlyPayout, color: COLORS.accent },
                { label: '💰 Total Return (Principal + Interest)', value: result.totalReturns, highlight: true, color: COLORS.warning },
              ]}
              disclaimer="Interest is taxable as per your slab and no TDS is deducted by Post Office. Reference rate only; verify the latest govt notification before investing. Not financial advice."
            />
            <DonutChart
